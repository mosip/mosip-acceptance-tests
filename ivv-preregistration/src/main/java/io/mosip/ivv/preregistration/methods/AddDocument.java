...
public class AddDocument extends Step implements StepInterface {
    ...
    public void run() {
        ...
        //ProofDocument proofDocument = this.store.getScenarioData().getPersona().getPersons().get(index)
        for (int j = 0; j < person.getDocuments().size(); j++) {
            ProofDocument proofDocument = this.store.getScenarioData().getPersona().getPersons().get(index).getDocuments().get(j);
